...
void parse(OBJTriangle &t, int i, const std::string &str) {
    ...
    if (tokens.size() == 1) {
        ...
        //obj Log(EError, "Invalid OBJ face format!");
    }
}
...
Texture *loadTexture(const FileResolver *fileResolver,
    ...
    if (!fs::exists(path)) {
        ...
        if (!fs::exists(path)) {
            //obj Log(EWarn, "Unable to find texture \"%s\" referenced from \"%s\"!",
                //obj path.string().c_str(), mtlPath.string().c_str());
...
void addMaterial(const std::string &name, Texture *diffuse, Texture *specular,
    ...
    bsdf->setID(name);
    //obj addChild(name, bsdf, false);
}

void loadMaterialLibrary(const FileResolver *fileResolver, const fs::path &mtlPath) {
    if (!fs::exists(mtlPath)) {
        //obj Log(EWarn, "Could not find referenced material library '%s'",
            //obj mtlPath.string().c_str());
        return;
    }

    //obj Log(EInfo, "Loading OBJ materials from \"%s\" ..", mtlPath.filename().string().c_str());
    fs::ifstream is(mtlPath);
    if (is.bad() || is.fail())
        ;//obj Log(EError, "Unexpected I/O error while accessing material file '%s'!",
            //obj mtlPath.string().c_str());
    std::string buf, line;
    std::string mtlName;
...
void createMesh(
    ...
    for (uint32_t i=0; i<triangles.size(); i++) {
        ...
        for (uint32_t j=0; j<3; j++) {
            ...
            if (vertexId > (int) vertices.size() || vertexId <= 0)
                ;//obj Log(EError, "Out of bounds: tried to access vertex %i (max: %i)", vertexId, (int) vertices.size());
            ...
            if (normalId != 0) {
                if (normalId > (int) normals.size() || normalId < 0)
                    ;//obj Log(EError, "Out of bounds: tried to access normal %i (max: %i)", normalId, (int) normals.size());
            ...
            if (uvId != 0) {
                if (uvId > (int) texcoords.size() || uvId < 0)
                    ;//obj Log(EError, "Out of bounds: tried to access uv %i (max: %i)", uvId, (int) texcoords.size());
    ...
    ref<TriMesh> mesh = new TriMesh(name,
        ...
        false, false);//obj m_flipNormals, m_faceNormals);
    ...
    //obj Log(EInfo, "%s: " SIZE_T_FMT " triangles, " SIZE_T_FMT
        //obj " vertices (merged " SIZE_T_FMT " vertices).", name.c_str(),
        //obj triangles.size(), vertexBuffer.size(), numMerged);
}
            ...
            std::vector<grain::KDTriangle*> &KDTriangles)
    {
        for(size_t i=0; i<meshes.size(); i++)
        {
            ...
            for(size_t j=0; j<count; j++) {
                KDTriangles.push_back(
                            new grain::KDTriangle(
...
#define NORUN 0
    bool rayIntersect(const Ray &_ray, Float mint, Float maxt, Float &t, void *temp) const {
        ...
        /*
        float tmin = 0., tout = 1.0/0.0;
        //bool res = m_mesh_kdtree->hit(ray, tout, tmin);
        bool res = (Grain*)(m_grains.at(0))->m_kdtree->hit(ray, tout, tmin);
        if(res) {
            t = (Float)tout;
        }
        */
        ...
        while(1) {
            while(1) { // loop only once, break at someplace, acts as "goto"
                ...
                if(occupy == 2){
                    float tmin = 0., tout = 1.0/0.0;
                    if ((tmin = 0., tout = 1.0/0.0, m_mesh_kdtree->hit(Ray(hitPoint, Vector(1,0,0), (Float)0), tout, tmin))
                     && (tmin = 0., tout = 1.0/0.0, m_mesh_kdtree->hit(Ray(hitPoint, Vector(-1,0,0), (Float)0), tout, tmin))
                     && (tmin = 0., tout = 1.0/0.0, m_mesh_kdtree->hit(Ray(hitPoint, Vector(0,1,0), (Float)0), tout, tmin))
                     && (tmin = 0., tout = 1.0/0.0, m_mesh_kdtree->hit(Ray(hitPoint, Vector(0,-1,0), (Float)0), tout, tmin))
                     && (tmin = 0., tout = 1.0/0.0, m_mesh_kdtree->hit(Ray(hitPoint, Vector(0,0,1), (Float)0), tout, tmin))
                     && (tmin = 0., tout = 1.0/0.0, m_mesh_kdtree->hit(Ray(hitPoint, Vector(0,0,-1), (Float)0), tout, tmin)))
                        ;
                    else {
                        // skip this sphere
